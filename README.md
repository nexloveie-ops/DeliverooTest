...
| `webhook` | Menu upload + `menu.upload_result` webhook (Scenario 6) | **GET live menu → mutate prices/descriptions → PUT** so payload differs from stored menu. Avoid `MATCH_EXISTING_MENU`. Webhook → `/webhooks/deliveroo`, poll `GET /deliveroo/menu/webhook-status?menuId=...` |
